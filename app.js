...
function loginSuccess(user, isRestoration = false) {
    ...
    if (user.role === 'admin' || user.role === 'developer' || user.role === 'provider') {
        ...
        if (user.role === 'admin' || user.role === 'developer') updateAdminPanel();
    }
...
window.showStep2 = function() {
    const name = document.getElementById('req-customer-name').value;
    ...
    if (!name || !dist || !town || !village || !serv) return alert("Please fill all details first.");
...
window.selectProvider = function (id, name) {
    ...
    btns.forEach(b => b.innerText = 'Select');
...
function updateProviderDashboard() {
    ...
    profileDiv.innerHTML = `
        <div style="padding:15px; border:1px solid var(--border); border-radius:var(--radius-sm); background:rgba(255,255,255,0.02);">
            ...
            <p><strong>Service:</strong> ${currentUser.providerService || 'Not Set'}</p>
    ...
    if (jobsList) {
        jobsList.innerHTML = '';
        const myJobs = requests.filter(r => r.providerId == currentUser.id);
        if (myJobs.length === 0) jobsList.innerHTML = '<p style="color:var(--text-muted); padding:2rem;">No jobs received yet.</p>';
        myJobs.forEach(job => {
            const div = document.createElement('div');
            ...
            div.innerHTML = `
                <div style="display:flex; justify-content:space-between; align-items:start;">
                    <h3>${job.service}</h3>
                    <span class="badge" style="background:var(--primary); color:#000; padding:4px 8px; border-radius:5px; font-size:0.7rem;">${job.id}</span>
                </div>
                <p><strong>Customer:</strong> ${job.customerName}</p>
                <p><strong>Phone:</strong> ${job.customerPhone}</p>
                <p><strong>Location:</strong> ${job.town}</p>
                <p><strong>Issue:</strong> ${job.desc}</p>
                <p><strong>Status:</strong> <span style="color:var(--primary)">${job.status.toUpperCase()}</span></p>
                <div class="mt-3" style="display:flex; gap:10px;">
                    ${job.status !== 'completed' ? `
                        <button onclick="markJobStatus('${job.id}', 'accepted')" class="btn-outline" style="flex:1;">Accept</button>
                        <button onclick="showJobCompletionModal('${job.id}')" class="btn-primary" style="flex:1;">Complete</button>
                    ` : `
                        <button class="btn-primary" style="flex:2" disabled>Job Completed ✅</button>
                        <button onclick="openRatingModal('${job.userId}', '${job.customerName}', 'customer')" class="btn-outline" style="flex:1">Rate ⭐</button>
                    `}
                </div>
            `;
...
function updateCustomerDashboard() {
    ...
    const profileDiv = document.getElementById('customer-profile-display');
    profileDiv.innerHTML = `
    ...
    // Job Tracking List
    const myJobs = requests.filter(r => r.userId == currentUser.id);
    const trackingRes = document.getElementById('tracking-result');
    if (trackingRes) {
        trackingRes.innerHTML = '<h4>My Recent Jobs</h4>';
        if (myJobs.length === 0) trackingRes.innerHTML += '<p style="color:var(--text-muted)">No jobs requested yet.</p>';
        myJobs.forEach(job => {
            const div = document.createElement('div');
            ...
            div.innerHTML = `
                ...
                <p>Status: <span style="color:var(--primary)">${job.status.toUpperCase()}</span></p>
                <p>Provider: ${job.providerName || 'Pending'}</p>
                <div class="mt-2">
                    ${job.status === 'completed' ? `<button onclick="openRatingModal('${job.providerId}', '${job.providerName}', 'provider')" class="btn-primary w-100">Rate Provider ⭐</button>` : ''}
                </div>
            `;
